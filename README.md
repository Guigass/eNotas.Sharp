...
Referência de API (Postman): `docs/API - eNotas - V2 - NF-e - NFC-e.postman_collection.json` (NF-e/NFC-e/empresas). NFS-e: coleção V1 `docs/API - eNotas - V1 - NFS-e.postman_collection.json` — no client C# há `EmitirNfse` (P1-02) e `ConsultaNfse` (P1-03); demais métodos NFS-e ainda no roadmap.
...
NFS-e: models `Nfse`/`Servico` (**P1-01**), `EmitirNfse` (**P1-02**) e `ConsultaNfse` (**P1-03**, path V1 `nfes/{nfeId}` — distinto de V2 `nf-e`/`nfc-e` e do model `Consulta`).
...
- Ainda não no client (ver [roadmap](docs/ROADMAP.md)):
    ...
    * NFS-e demais métodos (consultar por idExterno, cancelar, XML, PDF, listar, apoio municipal) — Postman V1 / itens P1-04+
    ```
