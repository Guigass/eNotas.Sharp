...
**Fato observado:** `eNotas.Sharp` é uma biblioteca C# (.NET Standard 2.0) que encapsula o consumo dos endpoints da API eNotas Gateway para NF-e, NFC-e (API V2) e NFS-e (API V1).
...
1. Expor um client público (`eNotasClient`) para emitir, consultar, cancelar e inutilizar NF-e/NFC-e, ciclo NFS-e V1, carta de correção, gestão de empresas/SAT e consulta de manifestação.
...
- Emissão NF-e / NFC-e (V2) e NFS-e (V1)
- Consulta de status e XML (e PDF NFS-e)
- Cancelamento
- Inutilização de numeração (NF-e/NFC-e)
- Carta de correção (NF-e)
- Apoio municipal NFS-e
...
- Empresas/SAT/consulta de manifestação (**P2-01..P2-11**, **P2-13** docs) e ciclo NFS-e V1 (**P1-01..P1-12**, docs README V1≠V2) já estão no client e no README. Envio de manifestação (**P2-12**) permanece bloqueado.
- Lacunas e priorização do backlog estão em `ROADMAP.md` (gaps restantes de models P0, envio de manifestação P2-12, qualidade P3).
...
4. Estudar `Models/Nota.cs` (NF-e/NFC-e) e `Models/Nfse.cs` / `Servico.cs` (NFS-e V1).
5. Consultar coleções Postman em `docs/` (V2 mercadorias; V1 NFS-e) — não misturar paths.
...
- Envio de manifestação (P2-12) — ver lista em `ROADMAP.md` (épico P1 Agente-pronto fechado).
