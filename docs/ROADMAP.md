...
Este documento **não** descreve features já disponíveis como se estivessem prontas. O envio de manifestação (P2-12) e parte dos métodos NFS-e (P1-11+) ainda não estão no `eNotasClient`. Já disponíveis: CRUD empresas/certificado/logo/desabilitar/habilitar/setup SAT/consultar SAT/consultar manifestação, emitir NFS-e (**P1-02**), consultar NFS-e por id GW (**P1-03**) e por idExterno (**P1-04**), listar NFS-e (**P1-05**), cancelar NFS-e por id GW (**P1-06**) e por idExterno (**P1-07**), download XML NFS-e por id GW e idExterno (**P1-08**), download PDF NFS-e por id GW e idExterno (**P1-09**), apoio municipal (**P1-10**).
...
- [x] `#region NFSe` (ou superfície equivalente) em `eNotasClient` — parcial: região + `EmitirNfse` (**P1-02**) + `ConsultaNfse` (**P1-03**) + `ConsultaNfsePorIdExterno` (**P1-04**) + `ListarNfse` (**P1-05**) + `CancelaNfse` (**P1-06**) + `CancelaNfsePorIdExterno` (**P1-07**) + `ConsultaNfseXML`/`ConsultaNfseXMLPorIdExterno` (**P1-08**) + `ConsultaNfsePDF`/`ConsultaNfsePDFPorIdExterno` (**P1-09**) + apoio municipal (**P1-10**); demais métodos P1-11+
- [x] Models request/response tipados — parcial: emissão (`Nfse`/`Servico`, **P1-01**) + consulta (`ConsultaNfse`, **P1-03**/**P1-04**) + lista (`ListaNfse`, **P1-05**); XML (**P1-08**) retorna string em `ApiResponse.Message` (sem schema tipado); PDF (**P1-09**) retorna `byte[]` em `ApiResponse<byte[]>.Object`; apoio municipal (**P1-10**) body em `Message` (schema Postman vazio)
- [ ] Documentação clara V1 ≠ V2
- [x] Exemplos/testes sem API Key real — parcial: `NfseSerializationTests` + fixture Postman + smoke `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno`/`ListarNfse`/`CancelaNfse`/`CancelaNfsePorIdExterno`/`ConsultaNfseXML`/`ConsultaNfseXMLPorIdExterno`/`ConsultaNfsePDF`/`ConsultaNfsePDFPorIdExterno`/`ConsultaServicosMunicipais`/`ConsultaServicosMunicipaisUnificados`/`ConsultaProvedorCidade`/`CriticarDadosObrigatorios` (sem API Key); exemplos de uso NFS-e ainda em P1-12
...
3. Postman V1 (NFS-e; `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno`/`ListarNfse`/`CancelaNfse`/`CancelaNfsePorIdExterno`/`ConsultaNfseXML`/`ConsultaNfseXMLPorIdExterno`/`ConsultaNfsePDF`/`ConsultaNfsePDFPorIdExterno`/apoio municipal P1-10 no client; demais P1-11+): `docs/API - eNotas - V1 - NFS-e.postman_collection.json`
...
- Tratar o ciclo NFS-e completo como disponível na lib (emitir + consultar + listar + cancelar + XML + PDF + apoio municipal estão no client; Reforma / docs P1-11+ ainda abertos).
