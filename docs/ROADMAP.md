...
Este documento **não** descreve features já disponíveis como se estivessem prontas. O envio de manifestação (P2-12) e a documentação NFS-e consolidada (P1-12) ainda não estão fechados. Já disponíveis: CRUD empresas/certificado/logo/desabilitar/habilitar/setup SAT/consultar SAT/consultar manifestação, emitir NFS-e (**P1-02**), consultar NFS-e por id GW (**P1-03**) e por idExterno (**P1-04**), listar NFS-e (**P1-05**), cancelar NFS-e por id GW (**P1-06**) e por idExterno (**P1-07**), download XML NFS-e por id GW e idExterno (**P1-08**), download PDF NFS-e por id GW e idExterno (**P1-09**), apoio municipal (**P1-10**), campos Reforma em `servico` (**P1-11**).
...
Models públicos em `eNotas.Sharp/Models/` cobrem emissão (`Nota`, `Iten`, impostos, pagamento, transporte, etc.), emissão NFS-e DTO (`Nfse`, `Servico` — **P1-01**; Reforma: `codigoNBS`/`codigoTributacaoNacional`/`ServicoIbsCbs` — **P1-11**), consulta NFS-e (`ConsultaNfse` — **P1-03**), lista NFS-e (`ListaNfse` — **P1-05**), consultas NF-e/NFC-e, XML fiscal e empresa (`Empresa`, `ConfiguracoesNfse`, `ListaEmpresas`).

**Fato:** `EmitirNfse` (**P1-02**), `ConsultaNfse` (**P1-03**), `ConsultaNfsePorIdExterno` (**P1-04**), `ListarNfse` (**P1-05**), `CancelaNfse` (**P1-06**), `CancelaNfsePorIdExterno` (**P1-07**), `ConsultaNfseXML` e `ConsultaNfseXMLPorIdExterno` (**P1-08**), `ConsultaNfsePDF` e `ConsultaNfsePDFPorIdExterno` (**P1-09**), apoio municipal (**P1-10**: `ConsultaServicosMunicipais`, `ConsultaServicosMunicipaisUnificados`, `ConsultaProvedorCidade`, `CriticarDadosObrigatorios`) estão no client; docs NFS-e (**P1-12**) e envio de manifestação (P2-12) ainda abertos/bloqueados. Models de emissão NFS-e (**P1-01**) e campos Reforma em `Servico` (**P1-11**) já existem. `SetupSat` (**P2-09**), `ConsultaSat` (**P2-10**) e `ConsultaManifestacao` (**P2-11**, host `api2`) já estão no client.
...
| P1-09 | Download PDF (id + idExterno) | `GET .../nfes/{nfeId}/pdf` e `.../porIdExterno/{idExterno}/pdf` | INFRA-01, P1-03 | **Agente-pronto** | Dois métodos retornando bytes via INFRA-01; [KB 173803](https://atendimento.notagateway.com.br/kb/pt-br/article/173803/baixar-o-pdf-ou-xml-de-uma-nota-fiscal) | Feito |
| P1-10 | Apoio municipal | `GET /v1/estados/{uf}/cidades/{nome}/servicos`, `GET /v1/servicos/cidades`, `GET /v1/estados/cidades/{codigoIBGECidade}/provedor`, `GET /v1/empresas/{empresaId}/criticardadosobrigatorios` | — | **Agente-pronto** | Métodos read-only tipados o suficiente; pode ser 1 commit por endpoint se preferir atomicidade | Feito |
| P1-11 | Campos Reforma em `servico` | — (DTO) | P1-01 | **Agente-pronto** (aditivo) | Adicionar ao model de serviço os campos do sample **Sandbox - Reforma**: `codigoNBS`, `codigoTributacaoNacional`, `ibsCbs` (shape do sample); nullable; sem apontar sandbox como base URL padrão | Feito |
...
- [x] `#region NFSe` (ou superfície equivalente) em `eNotasClient` — parcial: região + `EmitirNfse` (**P1-02**) + `ConsultaNfse` (**P1-03**) + `ConsultaNfsePorIdExterno` (**P1-04**) + `ListarNfse` (**P1-05**) + `CancelaNfse` (**P1-06**) + `CancelaNfsePorIdExterno` (**P1-07**) + `ConsultaNfseXML`/`ConsultaNfseXMLPorIdExterno` (**P1-08**) + `ConsultaNfsePDF`/`ConsultaNfsePDFPorIdExterno` (**P1-09**) + apoio municipal (**P1-10**) + campos Reforma em `Servico` (**P1-11**); docs **P1-12**
- [x] Models request/response tipados — parcial: emissão (`Nfse`/`Servico`, **P1-01**) + Reforma em `servico` (`codigoNBS`, `codigoTributacaoNacional`, `ServicoIbsCbs`, **P1-11**) + consulta (`ConsultaNfse`, **P1-03**/**P1-04**) + lista (`ListaNfse`, **P1-05**); XML (**P1-08**) retorna string em `ApiResponse.Message` (sem schema tipado); PDF (**P1-09**) retorna `byte[]` em `ApiResponse<byte[]>.Object`; apoio municipal (**P1-10**) body em `Message` (schema Postman vazio)
- [ ] Documentação clara V1 ≠ V2
- [x] Exemplos/testes sem API Key real — parcial: `NfseSerializationTests` + fixtures Postman (`nfse-emissao.json`, `nfse-reforma.json` **P1-11**) + smoke `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno`/`ListarNfse`/`CancelaNfse`/`CancelaNfsePorIdExterno`/`ConsultaNfseXML`/`ConsultaNfseXMLPorIdExterno`/`ConsultaNfsePDF`/`ConsultaNfsePDFPorIdExterno`/`ConsultaServicosMunicipais`/`ConsultaServicosMunicipaisUnificados`/`ConsultaProvedorCidade`/`CriticarDadosObrigatorios` (sem API Key); exemplos de uso NFS-e ainda em P1-12
...
| Postman V1 — NFS-e + apoio municipal + PDF | Models emissão (**P1-01** Feito: `Nfse`/`Servico`); Reforma em `servico` (**P1-11** Feito: `codigoNBS`/`codigoTributacaoNacional`/`ServicoIbsCbs`); `EmitirNfse` (**P1-02** Feito); `ConsultaNfse` (**P1-03** Feito: model `ConsultaNfse`); `ConsultaNfsePorIdExterno` (**P1-04** Feito); `ListarNfse` (**P1-05** Feito: model `ListaNfse`); `CancelaNfse` (**P1-06** Feito); `CancelaNfsePorIdExterno` (**P1-07** Feito); `ConsultaNfseXML`/`ConsultaNfseXMLPorIdExterno` (**P1-08** Feito — XML em `Message`); `ConsultaNfsePDF`/`ConsultaNfsePDFPorIdExterno` (**P1-09** Feito — bytes em `Object` via `GetBytes`); apoio municipal (**P1-10** Feito — 4 GETs, body em `Message`); **P1-12** aberto; **INFRA-01** Feito; evidência [KB 173803](https://atendimento.notagateway.com.br/kb/pt-br/article/173803/baixar-o-pdf-ou-xml-de-uma-nota-fiscal) |
...
3. Postman V1 (NFS-e; `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno`/`ListarNfse`/`CancelaNfse`/`CancelaNfsePorIdExterno`/`ConsultaNfseXML`/`ConsultaNfseXMLPorIdExterno`/`ConsultaNfsePDF`/`ConsultaNfsePDFPorIdExterno`/apoio municipal P1-10 / Reforma `servico` P1-11 no pacote; docs P1-12 aberto): `docs/API - eNotas - V1 - NFS-e.postman_collection.json`
...
- **PDF/XML NFS-e (V1):** [KB 173803](https://atendimento.notagateway.com.br/kb/pt-br/article/173803/baixar-o-pdf-ou-xml-de-uma-nota-fiscal) — XML no client (**P1-08**: `ConsultaNfseXML` / `ConsultaNfseXMLPorIdExterno`, body em `Message`); PDF no client (**P1-09**: `ConsultaNfsePDF` / `ConsultaNfsePDFPorIdExterno`, bytes em `Object` via `GetBytes`).

## Fora de escopo deste roadmap

- Implementar a feature em si (este arquivo só rastreia o backlog).
- Tratar o ciclo NFS-e completo como disponível na lib (emitir + consultar + listar + cancelar + XML + PDF + apoio municipal + campos Reforma em `servico` estão no pacote; docs P1-12 ainda aberto).
