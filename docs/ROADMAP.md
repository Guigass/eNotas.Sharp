...
Este documento **não** descreve features já disponíveis como se estivessem prontas. O envio de manifestação (P2-12) e parte dos métodos NFS-e (P1-10+) ainda não estão no `eNotasClient`. Já disponíveis: CRUD empresas/certificado/logo/desabilitar/habilitar/setup SAT/consultar SAT/consultar manifestação, emitir NFS-e (**P1-02**), consultar NFS-e por id GW (**P1-03**) e por idExterno (**P1-04**), listar NFS-e (**P1-05**), cancelar NFS-e por id GW (**P1-06**) e por idExterno (**P1-07**), download XML NFS-e por id GW e idExterno (**P1-08**), download PDF NFS-e por id GW e idExterno (**P1-09**).
...
3. Postman V1 (NFS-e; `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno`/`ListarNfse`/`CancelaNfse`/`CancelaNfsePorIdExterno`/`ConsultaNfseXML`/`ConsultaNfseXMLPorIdExterno`/`ConsultaNfsePDF`/`ConsultaNfsePDFPorIdExterno` no client; demais P1-10+): `docs/API - eNotas - V1 - NFS-e.postman_collection.json`
...
- Tratar o ciclo NFS-e completo como disponível na lib (emitir + consultar + listar + cancelar + XML + PDF estão no client; apoio municipal / Reforma / docs P1-10+ ainda abertos).
