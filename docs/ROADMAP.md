...
Este documento **não** descreve features já disponíveis como se estivessem prontas. O envio de manifestação (P2-12) e a maior parte dos métodos NFS-e (P1-05+) ainda não estão no `eNotasClient`. Já disponíveis: CRUD empresas/certificado/logo/desabilitar/habilitar/setup SAT/consultar SAT/consultar manifestação, emitir NFS-e (**P1-02**), consultar NFS-e por id GW (**P1-03**) e por idExterno (**P1-04**).
...
| NFS-e | Emitir (`EmitirNfse` → `POST /v1/empresas/{empresaId}/nfes`); Consultar por id GW (`ConsultaNfse` → `GET /v1/empresas/{empresaId}/nfes/{nfeId}`); Consultar por idExterno (`ConsultaNfsePorIdExterno` → `GET /v1/empresas/{empresaId}/nfes/porIdExterno/{idExterno}`); demais métodos P1-05+ ainda abertos |
...
Models públicos em `eNotas.Sharp/Models/` cobrem emissão (`Nota`, `Iten`, impostos, pagamento, transporte, etc.), emissão NFS-e DTO (`Nfse`, `Servico` — **P1-01**), consulta NFS-e (`ConsultaNfse` — **P1-03**), consultas NF-e/NFC-e, XML fiscal e empresa (`Empresa`, `ConfiguracoesNfse`, `ListaEmpresas`).

**Fato:** `EmitirNfse` (**P1-02**), `ConsultaNfse` (**P1-03**) e `ConsultaNfsePorIdExterno` (**P1-04**) estão no client; demais métodos NFS-e (P1-05+) e envio de manifestação (P2-12) ainda abertos/bloqueados. Models de emissão NFS-e (**P1-01**) já existem. `SetupSat` (**P2-09**), `ConsultaSat` (**P2-10**) e `ConsultaManifestacao` (**P2-11**, host `api2`) já estão no client.
...
- [x] `#region NFSe` (ou superfície equivalente) em `eNotasClient` — parcial: região + `EmitirNfse` (**P1-02**) + `ConsultaNfse` (**P1-03**) + `ConsultaNfsePorIdExterno` (**P1-04**); demais métodos P1-05+
- [x] Models request/response tipados — parcial: emissão (`Nfse`/`Servico`, **P1-01**) + consulta (`ConsultaNfse`, **P1-03**/**P1-04**); lista ainda em P1-05
- [ ] Documentação clara V1 ≠ V2
- [x] Exemplos/testes sem API Key real — parcial: `NfseSerializationTests` + fixture Postman + smoke `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno` (sem API Key); exemplos de uso NFS-e ainda em P1-12
...
3. Postman V1 (NFS-e; `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno` no client; demais P1-05+): `docs/API - eNotas - V1 - NFS-e.postman_collection.json`
...
- Tratar o ciclo NFS-e completo como disponível na lib (só emitir + consultar por id GW/idExterno estão no client).
