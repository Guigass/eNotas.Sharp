...
Este documento **não** descreve features já disponíveis como se estivessem prontas. NFS-e e o restante da gestão de empresas (cert/logo/SAT/habilitar) estão no Postman / README, mas **ainda não** no `eNotasClient` (consultar/incluir-alterar/listar empresa já estão).
...
| Empresas | Incluir/Alterar (`IncluirAlterarEmpresa` → `POST /v2/empresas`); Consultar por id (`ConsultaEmpresa` → `GET /v2/empresas/{empresaId}`); Listar (`ListarEmpresas` → `GET /v2/empresas?...`) |
...
**Fato:** não há métodos de NFS-e, certificado, logo, SAT nem manifestação no client (P2-05+ / P1 abertos).
...
| Postman V2 — pasta empresas (CRUD, certificado, logo, SAT) | Models `Empresa`/`ConfiguracoesNfse`/`ListaEmpresas` (**P2-01** Feito); `IncluirAlterarEmpresa` (**P2-02** Feito); `ConsultaEmpresa` (**P2-03** Feito); `ListarEmpresas` (**P2-04** Feito); métodos client **P2-05..P2-10**, **P2-13** abertos; pré-req **INFRA-02** Feito |
