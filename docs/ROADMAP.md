...
Este documento **não** descreve features já disponíveis como se estivessem prontas. NFS-e e o envio de manifestação (P2-12) estão no Postman / README, mas **ainda não** no `eNotasClient` (CRUD empresas/certificado/logo/desabilitar/habilitar/setup SAT/consultar SAT/consultar manifestação já estão).
