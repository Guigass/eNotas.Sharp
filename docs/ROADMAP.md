...
Este documento **não** descreve features já disponíveis como se estivessem prontas. O envio de manifestação (P2-12) ainda não está fechado. Já disponíveis: CRUD empresas/certificado/logo/desabilitar/habilitar/setup SAT/consultar SAT/consultar manifestação, ciclo NFS-e V1 (**P1-01..P1-12**: models, emitir, consultar, listar, cancelar, XML, PDF, apoio municipal, Reforma em `servico`, docs).
...
| NFS-e | Ciclo V1 completo no `#region NFSe` (**P1-01..P1-12** Feito): Emitir (`EmitirNfse` → `POST /v1/empresas/{empresaId}/nfes`); Consultar por id GW (`ConsultaNfse`); por idExterno (`ConsultaNfsePorIdExterno`); Listar (`ListarNfse`); Cancelar por id GW / idExterno; XML (`ConsultaNfseXML` / `ConsultaNfseXMLPorIdExterno`, body em `Message`); PDF (`ConsultaNfsePDF` / `ConsultaNfsePDFPorIdExterno`, bytes em `Object`); Apoio municipal (**P1-10**): `ConsultaServicosMunicipais`, `ConsultaServicosMunicipaisUnificados`, `ConsultaProvedorCidade`, `CriticarDadosObrigatorios` (body em `Message`); Reforma em `Servico` (**P1-11**); docs README/seção V1≠V2 (**P1-12**) |
...
**Fato:** ciclo NFS-e V1 (**P1-01..P1-12**) está no client e documentado (README seção NFS-e ≠ NF-e/NFC-e). Envio de manifestação (**P2-12**) permanece bloqueado. `SetupSat` (**P2-09**), `ConsultaSat` (**P2-10**) e `ConsultaManifestacao` (**P2-11**, host `api2`) já estão no client.
...
| P1-12 | Docs NFS-e | — | P1-02..P1-09 (mínimo emitir+consulta+cancel+xml) | **Agente-pronto** | README: seção NFS-e ≠ NF-e/NFC-e; lista de métodos reais; sem API Key; `docs/` alinhados (`MODULES`/`ARCHITECTURE` se tocados) | Feito |

**Critério de aceite do épico P1** (fechado quando todos Agente-pronto acima estiverem Feito):

- [x] `#region NFSe` (ou superfície equivalente) em `eNotasClient` — região + emitir/consultar/listar/cancelar/XML/PDF + apoio municipal + Reforma em `Servico` + docs (**P1-01..P1-12**)
- [x] Models request/response tipados — emissão (`Nfse`/`Servico`, **P1-01**) + Reforma (`codigoNBS`, `codigoTributacaoNacional`, `ServicoIbsCbs`, **P1-11**) + consulta (`ConsultaNfse`) + lista (`ListaNfse`); XML em `Message`; PDF em `byte[]`/`Object`; apoio municipal body em `Message`
- [x] Documentação clara V1 ≠ V2 — README seção NFS-e + tabela Postman V1/V2; `MODULES`/`ARCHITECTURE`/`PROJECT_OVERVIEW`/`DOMAIN` alinhados (**P1-12**)
- [x] Exemplos/testes sem API Key real — `NfseSerializationTests` + fixtures + smoke de paths; exemplo de uso NFS-e no README (placeholder `apiKey`/`empresaId`, sem secret)
...
| Postman V1 — NFS-e + apoio municipal + PDF | Ciclo **P1-01..P1-12** Feito (models, emitir/consultar/listar/cancelar, XML/PDF, apoio municipal, Reforma, docs README V1≠V2); **INFRA-01** Feito; evidência [KB 173803](https://atendimento.notagateway.com.br/kb/pt-br/article/173803/baixar-o-pdf-ou-xml-de-uma-nota-fiscal) |
...
3. Postman V1 (NFS-e; ciclo **P1-01..P1-12** Feito no pacote + README): `docs/API - eNotas - V1 - NFS-e.postman_collection.json`
...
- Inventar métodos NFS-e além do inventário real em `#region NFSe` / README (ciclo **P1-01..P1-12** Feito).
