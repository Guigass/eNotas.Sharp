...
Este documento **não** descreve features já disponíveis como se estivessem prontas. O envio de manifestação (P2-12) e a maior parte dos métodos NFS-e (P1-07+) ainda não estão no `eNotasClient`. Já disponíveis: CRUD empresas/certificado/logo/desabilitar/habilitar/setup SAT/consultar SAT/consultar manifestação, emitir NFS-e (**P1-02**), consultar NFS-e por id GW (**P1-03**) e por idExterno (**P1-04**), listar NFS-e (**P1-05**), cancelar NFS-e por id GW (**P1-06**).
...
**Fato:** `EmitirNfse` (**P1-02**), `ConsultaNfse` (**P1-03**), `ConsultaNfsePorIdExterno` (**P1-04**), `ListarNfse` (**P1-05**) e `CancelaNfse` (**P1-06**) estão no client; demais métodos NFS-e (P1-07+) e envio de manifestação (P2-12) ainda abertos/bloqueados. Models de emissão NFS-e (**P1-01**) já existem. `SetupSat` (**P2-09**), `ConsultaSat` (**P2-10**) e `ConsultaManifestacao` (**P2-11**, host `api2`) já estão no client.
...
- [x] `#region NFSe` (ou superfície equivalente) em `eNotasClient` — parcial: região + `EmitirNfse` (**P1-02**) + `ConsultaNfse` (**P1-03**) + `ConsultaNfsePorIdExterno` (**P1-04**) + `ListarNfse` (**P1-05**) + `CancelaNfse` (**P1-06**); demais métodos P1-07+
...
- [x] Exemplos/testes sem API Key real — parcial: `NfseSerializationTests` + fixture Postman + smoke `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno`/`ListarNfse`/`CancelaNfse` (sem API Key); exemplos de uso NFS-e ainda em P1-12
...
3. Postman V1 (NFS-e; `EmitirNfse`/`ConsultaNfse`/`ConsultaNfsePorIdExterno`/`ListarNfse`/`CancelaNfse` no client; demais P1-07+): `docs/API - eNotas - V1 - NFS-e.postman_collection.json`
...
- Tratar o ciclo NFS-e completo como disponível na lib (só emitir + consultar por id GW/idExterno + listar + cancelar por id GW estão no client).
