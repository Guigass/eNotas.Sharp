...
- Models de emissão → composição (`Nota` → `Cliente`, `Iten`, `Impostos`, `Pedido`, `Transporte`, …); NFS-e V1 → `Nfse` → `Cliente`, `Servico` (**P1-01**; `EmitirNfse` / **P1-02**); consulta NFS-e → `ConsultaNfse` (**P1-03**; ≠ `Consulta` de NF-e/NFC-e)
...
- Coleção Postman V1 (NFS-e) é referência de contrato para o backlog **P1**; DTO emissão (`Nfse`/`Servico`, **P1-01**), `EmitirNfse` (**P1-02**) e `ConsultaNfse` (**P1-03**) no client; demais métodos P1-04+.
