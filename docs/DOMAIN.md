...
| NFS-e | `Nfse.cs`, `Servico.cs`, `ConsultaNfse.cs`, `ListaNfse.cs` | Emissão/consulta/lista V1 (`/v1/.../nfes`); request `enviarPorEmail` (KB 170286), opcionais `numeroRps`/`serieRps` (KB 173801); retorno `enviadaPorEmail` em `ConsultaNfse`; ≠ `Nota`/`Consulta` V2. **Request `Nfse`/`Servico` (v1.30.0):** cobre `dataCompetencia`, `naturezaOperacao`, `observacoes`, `dadosAdicionaisEmail`, `deducoes`/`descontos`/`descontoCondicionado` e campos federais/ISS de `Servico` (`exportacao`, `regimeEspecialTributacao`, `tipoImunidadeIss`, `paisPrestacaoServico`, `ufPrestacaoServico`, `exigibilidadeSuspensa`, `pisCofinsApuracaoPropria`, retenções `valorPis`/`valorCofins`/`valorCsll`/`valorInss`/`valorIr`, `situacaoTributariaPisCofins`, `tipoRetencaoPisCofins`), igualando os campos já presentes em `ConsultaNfse` como response. |
