...
1. Serialização/deserialização JSON (`Nota`, `Consulta`, `Inutilizacao`, `CartaCorrecao`, `Impostos`/`IbsCbs` completo e simplificado, `Empresa`/`ConfiguracoesNfse`/`ListaEmpresas`, `Nfse`/`Servico` + Reforma `ServicoIbsCbs`, `ConsultaNfse`/`ListaNfse` via smoke) e `CustomDateTimeConverter`
