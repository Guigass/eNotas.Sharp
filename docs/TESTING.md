...
| HTTP smoke | `eNotasClientPathTests` | Métodos NF-e/NFC-e + NFS-e (`EmitirNfse` → `POST /v1/.../nfes`) + empresas (`IncluirAlterarEmpresa`, `ConsultaEmpresa`, `ListarEmpresas`, multipart cert/logo, desabilitar/habilitar, `SetupSat`, `ConsultaSat`, `ConsultaManifestacao` host `api2`/`v3`); auth Basic; sucesso e 4xx; `CancellationToken` cancelado |
