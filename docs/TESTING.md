...
1. Serialização/deserialização JSON (`Nota`, `Consulta`, `Inutilizacao`, `CartaCorrecao`, `Impostos`/`IbsCbs` completo e simplificado, `Empresa`/`ConfiguracoesNfse`/`ListaEmpresas`, `Nfse`/`Servico` + Reforma `ServicoIbsCbs`, `ConsultaNfse`/`ListaNfse`) e `CustomDateTimeConverter`
...
|------|-------|------------|
| Serialização JSON | `NotaSerializationTests`, `ConsultaSerializationTests`, `ConsultaNfseSerializationTests`, `InutilizacaoSerializationTests`, `CartaCorrecaoSerializationTests`, `ImpostosSerializationTests`, `EmpresaSerializationTests`, `NfseSerializationTests`, `CustomDateTimeConverterTests` | Fixtures em `eNotas.Sharp.Tests/Fixtures/` (incl. `empresa-incluir-alterar.json`, `nfse-emissao.json`, `nfse-reforma.json`, `consulta-nfse.json`, `lista-nfse.json`). RoundTrip NFS-e: `JToken.DeepEquals` após strip de nulls (`*_PreservesJsonShape`) |
| XML | `NfeProcXmlTests`, `XmlDocumentTests` | Fixtures mínimas, não documento fiscal completo |
| HTTP smoke | `eNotasClientPathTests` | Métodos NF-e/NFC-e + NFS-e (`EmitirNfse` → `POST /v1/.../nfes`; `ConsultaNfse` → `GET /v1/.../nfes/{nfeId}`; `ConsultaNfsePorIdExterno` → `GET /v1/.../nfes/porIdExterno/{idExterno}`; `ListarNfse` → `GET /v1/.../nfes?...`; `CancelaNfse` → `DELETE /v1/.../nfes/{nfeId}`; `CancelaNfsePorIdExterno` → `DELETE /v1/.../nfes/porIdExterno/{idExterno}`; `ConsultaNfseXML` → `GET /v1/.../nfes/{nfeId}/xml` (XML em `Message`); `ConsultaNfseXMLPorIdExterno` → `GET /v1/.../nfes/porIdExterno/{idExterno}/xml`; `ConsultaNfsePDF` → `GET /v1/.../nfes/{nfeId}/pdf` (bytes em `Object`); `ConsultaNfsePDFPorIdExterno` → `GET /v1/.../nfes/porIdExterno/{idExterno}/pdf`; Theory `NfsePorIdExterno_EscapesIdExternoInPath` (espaço/`%`); apoio municipal **P1-10**: `ConsultaServicosMunicipais` → `GET /v1/estados/{uf}/cidades/{nome}/servicos?...`; `ConsultaServicosMunicipaisUnificados` → `GET /v1/servicos/cidades?...`; `ConsultaProvedorCidade` → `GET /v1/estados/cidades/{codigoIBGECidade}/provedor`; `CriticarDadosObrigatorios` → `GET /v1/empresas/{empresaId}/criticardadosobrigatorios` — body em `Message`) + empresas (`IncluirAlterarEmpresa`, `ConsultaEmpresa`, `ListarEmpresas`, multipart cert/logo, desabilitar/habilitar, `SetupSat`, `ConsultaSat`, `ConsultaManifestacao` host `api2`/`v3`); auth Basic; sucesso e 4xx; `CancellationToken` cancelado |
