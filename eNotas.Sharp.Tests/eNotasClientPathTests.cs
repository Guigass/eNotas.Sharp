...
public class eNotasClientPathTests
{
    ...
    [Fact]
    public async Task ConsultaNfse_GetsTypedConsultaNfse()
    {
        const string json = """
            {
              "id": "nota-123",
        ...
        using (client)
        {
            var response = await client.ConsultaNfse(ClientTestFactory.NotaId, ClientTestFactory.EmpresaId);

            Assert.True(response.IsSuccess);
            Assert.Equal(HttpMethod.Get, handler.LastRequest!.Method);
            Assert.EndsWith(
                $"/v1/empresas/{ClientTestFactory.EmpresaId}/nfes/{ClientTestFactory.NotaId}",
                handler.LastRequest.RequestUri!.AbsolutePath);
            Assert.Equal($"Basic {ClientTestFactory.ApiKey}", handler.LastRequest.Headers.GetValues("Authorization").Single());
            Assert.Equal("nota-123", response.Object!.Id);
            Assert.Equal("NFS-e", response.Object.Tipo);
            Assert.Equal("TESTE231024", response.Object.IdExterno);
    ...
    [Fact]
    public async Task CancelaNfse_DeletesExpectedPath()
    {
        var (client, handler) = ClientTestFactory.Create(_ => ClientTestFactory.Ok("{\"ok\":true}"));
        using (client)
        {
            var response = await client.CancelaNfse(ClientTestFactory.NotaId, ClientTestFactory.EmpresaId);
    ...
    [Fact]
    public async Task ConsultaNfseXML_GetsExpectedPathWithXmlInMessage()
    {
        const string xmlBody = "<?xml version=\"1.0\"?><Nfse><Numero>1</Numero></Nfse>";
        var (client, handler) = ClientTestFactory.Create(_ => ClientTestFactory.Ok(xmlBody));
        using (client)
        {
            var response = await client.ConsultaNfseXML(ClientTestFactory.NotaId, ClientTestFactory.EmpresaId);

            Assert.True(response.IsSuccess);
            Assert.Equal("OK", response.Status);
            Assert.Equal(xmlBody, response.Message);
            Assert.Equal(HttpMethod.Get, handler.LastRequest!.Method);
            Assert.EndsWith(
                $"/v1/empresas/{ClientTestFactory.EmpresaId}/nfes/{ClientTestFactory.NotaId}/xml",
                handler.LastRequest.RequestUri!.AbsolutePath);
            Assert.Equal($"Basic {ClientTestFactory.ApiKey}", handler.LastRequest.Headers.GetValues("Authorization").First());
    ...
    [Fact]
    public async Task ConsultaNfseXMLPorIdExterno_GetsExpectedPathWithXmlInMessage()
    {
        const string idExterno = "TESTE231024";
        const string xmlBody = "<?xml version=\"1.0\"?><Nfse><Numero>1</Numero></Nfse>";
        var (client, handler) = ClientTestFactory.Create(_ => ClientTestFactory.Ok(xmlBody));
        using (client)
        {
            var response = await client.ConsultaNfseXMLPorIdExterno(idExterno, ClientTestFactory.EmpresaId);
            ...
            Assert.Equal(HttpMethod.Get, handler.LastRequest!.Method);
            Assert.EndsWith(
                $"/v1/empresas/{ClientTestFactory.EmpresaId}/nfes/porIdExterno/{idExterno}/xml",
                handler.LastRequest.RequestUri!.AbsolutePath);
            Assert.Equal($"Basic {ClientTestFactory.ApiKey}", handler.LastRequest.Headers.GetValues("Authorization").First());
            Assert.Null(handler.LastRequest.Content);
        }
    ...
    public async Task SetupSat_GetsExpectedPathWithAuth()
    {
        ...
        using (client)
        {
            var response = await client.SetupSat(ClientTestFactory.EmpresaId);
            ...
            Assert.Equal(HttpMethod.Get, handler.LastRequest!.Method);
            Assert.EndsWith(
                $"/v2/empresas/{ClientTestFactory.EmpresaId}/sat/setup",
                handler.LastRequest.RequestUri!.AbsolutePath);
            Assert.Equal($"Basic {ClientTestFactory.ApiKey}", handler.LastRequest.Headers.GetValues("Authorization").Single());
            Assert.Null(handler.LastRequest.Content);
        }
    ...
    [Fact]
    public async Task ConsultaManifestacao_GetsAbsoluteApi2HostAndPathWithAuth()
    {
        var (client, handler) = ClientTestFactory.Create(_ => ClientTestFactory.Ok("{\"ok\":true}"));
        using (client)
        {
            var response = await client.ConsultaManifestacao(
                ClientTestFactory.ChaveAcesso,
                ClientTestFactory.EmpresaId);
            ...
            Assert.Equal("{\"ok\":true}", response.Message);
            Assert.Equal(HttpMethod.Get, handler.LastRequest!.Method);
            Assert.Equal("api2.enotasgw.com.br", handler.LastRequest.RequestUri!.Host);
            Assert.Equal(
                $"/v3/empresas/{ClientTestFactory.EmpresaId}/nf-e/manifestacao/{ClientTestFactory.ChaveAcesso}",
                handler.LastRequest.RequestUri.AbsolutePath);
            Assert.Equal(
                $"https://api2.enotasgw.com.br/v3/empresas/{ClientTestFactory.EmpresaId}/nf-e/manifestacao/{ClientTestFactory.ChaveAcesso}",
                handler.LastRequest.RequestUri.AbsoluteUri);
            Assert.Equal($"Basic {ClientTestFactory.ApiKey}", handler.LastRequest.Headers.GetValues("Authorization").Single());
            Assert.Null(handler.LastRequest.Content);
        }
